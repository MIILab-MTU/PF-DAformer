...
class SegFormer3D(nn.Module):
    def __init__(
        ...
        self.domain_classifier = DomainClassifier(embed_dims_last=384)  # Adjust embed_dims_last
    ...
    def forward(self, x, return_features=False,use_domain_adapt=False):
        ...
        if return_features:
            if use_domain_adapt:
                ...
                # print("c4 requires grad?", c4.requires_grad)
                domain_pred = self.domain_classifier(domain_feat)
                # print("domain_feat shape:", domain_feat.shape)
                return output, c4, domain_pred
            else:
                return output, c4  # or concatenate features if needed
        return output
