...
def build_swin3d_segmentation_model():
    config = SwinTransformerConfig3D(
        input_size=(192, 192, 192),  # Keep this unless you crop inputs
        in_channels=1,
        embed_dim=32,  # ↓ Reduce from 48 → 32 to cut memory across all layers
        num_blocks=[1, 1, 2, 2],  # ↓ Shallower early layers reduce computation
        patch_window_size=[(2, 2, 2)] * 4,  # ↑ Larger patches reduce token count
        block_window_size=[(6, 6, 6)] * 4,
        num_heads=[2, 4, 8, 16],  # Keep this aligned with embed_dim
        mlp_ratio=2.0,  # Lighter feedforward
        qkv_bias=True,
        drop=0.0,
        drop_attn=0.0,
        drop_path=0.05,
    ...
    model = Swin3DSegmentationModel(config, n_classes=1)
    return model
...
class Swin3DSegmentationModel(nn.Module):
    def __init__(self, config: SwinTransformerConfig3D, n_classes: int = 1):
        super().__init__()
        self.config = config
        self.backbone = SwinTransformer3D(self.config)
        self.final_out_channels = self.backbone.out_channels[-1]
        # self.decoder = nn.Sequential(
        #     nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False),
        #     nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False),
        #     nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False),
        #     nn.Upsample(scale_factor=2, mode="trilinear", align_corners=False),
        #     nn.Conv3d(self.final_out_channels, n_classes, kernel_size=1)
        # )
        self.decoder = nn.Sequential( 
            nn.ConvTranspose3d(self.final_out_channels, 64, kernel_size=2, stride=2),
            ...
            nn.ReLU(inplace=True),
            nn.Conv3d(8, n_classes, kernel_size=1)
        )
        self.domain_classifier = DomainClassifier(self.final_out_channels)
    ...
    def forward(self, x, return_features=False, use_domain_adapt=False):
        ...
        B, N, C_feat = c4.shape
        D_out, H_out, W_out = self.backbone.output_size[-1]
        x = c4.permute(0, 2, 1).contiguous().view(B, C_feat, D_out, H_out, W_out)
        output = self.decoder(x)
        # output = output.squeeze(1) if output.shape[1] == 1 else output
        if return_features:
            if use_domain_adapt:
                domain_feat = grad_reverse(c4)
                domain_pred = self.domain_classifier(
                    domain_feat.permute(0, 2, 1).contiguous().view(B, C_feat, D_out, H_out, W_out)
                )
                return output, c4, domain_pred
            else:
                return output, c4
        return output

model = build_swin3d_segmentation_model()
x = torch.randn(1, 1, 192, 192, 192)  # Input
y = model(x)

print("Input shape:", x.shape)
print("Output shape:", y.shape)
# print("Output Y1 shape:", y1.shape)
# print("Output Y2 shape:", y1.shape)
