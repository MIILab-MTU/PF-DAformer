...
def launch_experiment(config_path) -> Dict:
    ...
    # run train
    if config["training_parameters"]["load_checkpoint"]["load_full_checkpoint"] == True:
        trainer._update_scheduler()
...
def build_directories(config: Dict) -> None:
    ...
    if os.listdir(config["training_parameters"]["checkpoint_save_dir"]) and config["training_parameters"]["load_checkpoint"]["load_full_checkpoint"] == False:
